...
### P1 封面
- 大标题（84px）用汇文明朝体（Huiwen Mincho），关键词用主色高亮块（`background: var(--brand-primary); color: #fff; padding: 4px 16px; border-radius: 6px`）
- 副标题（44px）一行显示，紧跟标题下方，`white-space: nowrap`
- 圆形头像（`avatar.jpg`，120px，`border: 4px solid var(--brand-accent)`）
- 署名（作者名）44px + 介绍34px
- 整体边框：`border: 28px solid var(--brand-accent)`
...
- 品牌三色比例：主色6 : 强调3 : 点缀1
- 背景主色：奶白 `var(--cream)` / 深奶 `var(--cream-dark)`，深色面板用 `var(--ink)`
